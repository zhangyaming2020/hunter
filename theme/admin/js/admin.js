...
;$(function($){
	...
	//普通弹出
	$('.dialog_open').live('click', function(){
		var self = $(this),
			dtitle = self.attr('data-title'),
			did = self.attr('data-id'),
			duri = self.attr('data-uri'),
	...
	//详情弹出
	$('.dialog_detail').live('click', function(){
		var self = $(this),
			dtitle = self.attr('data-title'),
			did = self.attr('data-id'),
			duri = self.attr('data-uri'),
...
;(function($){
    //联动菜单
    $.fn.cate_select = function(options,get_cate_id,admin) {
        var settings = {
        ...
        $('.'+settings.target_class).die('change').live('change', function(){
            ...
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                	if(result.type == 2){
                        var _childs = $('<select class="'+settings.target_class+' mr10" data-pid="'+_pid+'" style="display:none;"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                ...
                if(admin == true)
                	get_cate_id(_this.attr('data-pid'));
            }
        });
    }
})(jQuery);
