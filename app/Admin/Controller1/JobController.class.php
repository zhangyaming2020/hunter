...
use Admin\Org\Tree;
class JobController extends AdminCoreController {
    ...
    public function index() {
        ...
        foreach($result as $r) {
            ...
            $r['str_manage'] = '<a href="javascript:;" data-uri="'.U('Job/add',array('pid'=>$r['id'])).'" data-title="'.L('add_item_cate').'"   class="J_showdialog btn btn-xs btn-success btn-add" title="添加子分类" data-id="add" data-width="520" data-height="20"><i class="fa fa-plus"></i></a>
                                <a href="javascript:;" data-uri="'.U('Job/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="520" data-height="20" class="J_showdialog btn btn-xs btn-success btn-editone" title="'.L('edit').'"><i class="fa fa-pencil"></i></a>
                                <a href="javascript:;" data-acttype="ajax" data-uri="'.U('Job/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'" class="J_confirmurl btn btn-xs btn-danger btn-delone" title="'.L('delete').'"><i class="fa fa-trash"></i></a>';
    ...
    public function _before_add(){
        ...
        $this->assign('Job_type',C('Job_type'));
    }
    ...
    {
        $id = I('id', '', 'intval');
        $Job = $this->_mod->field('id,pid')->where(array('id' => $id))->find();
        $Job_spid = M('Job')->where(array('id' => $Job['pid']))->getField('spid');
        if ($Job_spid == 0) {
            $Job_spid = $Job['pid'];
        } else {
            $Job_spid .= $Job['pid'];
    ...
    protected function _before_update($data) {
        ...
        $Job = $this->_mod->field('pid')->where(array('id'=>$data['id']))->find();
        if ($data['pid'] != $Job['pid']) {
    ...
    public function get_city_info(){
        ...
        //$url = 'http://api.map.baidu.com/Job/v2/suggestion?query='.$kw.'&region=136&output=json&ak='.C('baidu_lbs_key');
