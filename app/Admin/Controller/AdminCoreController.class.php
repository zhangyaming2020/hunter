...
class AdminCoreController extends StoneController {
    ...
    public function check_priv() {
    	if ( (!isset($_SESSION['admin']) || !$_SESSION['admin']) && !in_array(ACTION_NAME, array('login','verify_code')) ) {
            if(IS_AJAX){
            	echo "<script>alert('登录已过期,请刷新整个窗口')</script>";
            }
            $this->redirect('index/login');
        }
        if (in_array(CONTROLLER_NAME,array('Place','Candidate','Attachment','Email','Progress','Linkman','Custom','Project','Resume','Contract','Offer','Report'))) {
