...
use Think\Model;
use Common\Org\Dir;
class BackupController extends AdminCoreController {
    ...
    public function index()
    {
        if (IS_POST || isset($_GET['dosubmit']))
        {
            ...
            } else
            {
                ...
                if (is_dir($this->data_path . $this->backup_path . $this->backup_name))
                {
                    $this->error(L('backup_name') . L('exists'));
                }
            	//mkdir($this->data_path . $this->backup_path . $this->backup_name,0777,true);
				mkdir($this->data_path . $this->backup_path . $this->backup_name,0777,true);
                if (!is_file($this->data_path . $this->backup_path . $this->backup_name .
                    '/tbl_queue.log'))
                {
                    //写入队列
    ...
    public function import()
    {
        ...
        if ($this->_import_vol($backup['file']))
        {
            ...
            {
                $vol++;
                $link = 'admin.php?m=admin&c=' .CONTROLLER_NAME. '&a=import&vol=' . $vol .
                    '&backup=' . urlencode($this->backup_name);
                $this->success(sprintf(L('bakup_import_succ'), $vol - 1), $link);
            } else
            {
                $this->success(L('restore_success'), 'admin.php?m=admin&c=' . CONTROLLER_NAME .
                    '&a=restore');
            }
        }
    }
    ...
    {
        $sql_file = $this->data_path . $this->backup_path . $this->backup_name . '/' . $sql_file_name;
    ...
    public function del_backup()
    {
        ...
        $dir=new Dir();
        $dir->delDir($this->data_path . $this->backup_path . $_GET['backup'].'/');             
    ...
    public function download()
    {
        ...
        $sql_file = $this->data_path . $this->backup_path . $backup_name . '/' . $file;
    ...
    private function _get_vols($backup_name)
    {
        $vols = array(); //所有的卷
        $bytes = 0;
        $vol_path = $this->data_path . $this->backup_path . $backup_name . '/';
        if (is_dir($vol_path))
        {
    ...
    {
        $backups = array(); //所有的备份
        if (is_dir($this->data_path . $this->backup_path))
        {
            if ($handle = opendir($this->data_path . $this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    if ($file{0} != '.' && filetype($this->data_path . $this->backup_path . $file) == 'dir')
                    {
                        $backup['name'] = $file;
                        $backup['date'] = filemtime($this->data_path . $this->backup_path . $file) - date('Z');
                        $backup['date_str'] = date('Y-m-d H:i:s', $backup['date']);
                        $backup['vols'] = $this->_get_vols($file);
                        $end_vol = end($backup['vols']);
                        $backup['total_size'] =round($this->_get_dir_size($this->data_path . $this->backup_path . $file)/1024,2);
                        $backups[] = $backup;
                    }
                }
            }
    ...
    private function _deal_result($tables, $vol, $sizelimit)
    {
        ...
        if (empty($tables))
        {
            ...
            $this->success(L('bakup_succ'), 'admin.php?m=admin&c=' . CONTROLLER_NAME .
                '&a=restore');
        } else
        {
            //开始下一卷
            $this->_set_vol($vol); //设置分卷记录
            $link = 'admin.php?m=admin&c=' . CONTROLLER_NAME .
    ...
    {
        $post_pos = $pos;
        $total = M('')->query("SELECT COUNT(*) as num FROM $table"); //数据总数
        $total = $total[0]['num'];
    ...
    {
        $backup_path = $this->data_path . '/data/backup/';
    ...
    {
        return file_put_contents($this->data_path . $this->backup_path . $this->backup_name .
    ...
    {
        $tbl_queue_file = $this->data_path . $this->backup_path . $this->backup_name .
            '/tbl_queue.log';
    ...
    {
        $log_file = $this->data_path . $this->backup_path . $this->backup_name . '/vol.log';
