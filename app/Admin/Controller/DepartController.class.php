...
use Admin\Org\Tree;
class DepartController extends AdminCoreController {
    ...
    public function index() {
        ...
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        if(F('depart_list')){
            $result = F('depart_list');
        }else{
            $result = $this->_mod->order($sort . ' ' . $order)->select();
            F('depart_list',$result);
        }
        
        $array = array();
        //dump($result);exit;
        foreach($result as $r) {
            $r['str_status'] = '<img data-tdtype="toggle" data-id="'.$r['id'].'" data-field="status" data-value="'.$r['status'].'" src="theme/admin/images/toggle_' . ($r['status'] == 0 ? 'disabled' : 'enabled') . '.gif" />';
            $r['str_manage'] = '<a href="javascript:;" data-uri="'.U('depart/add',array('pid'=>$r['id'])).'" data-title="'.L('add_item_cate').'"   class="J_showdialog" title="添加子分类" data-id="add" data-width="520" data-height="20">'.L('add_submenu').'</a>|
                                <a href="javascript:;" data-uri="'.U('depart/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="520" data-height="20" class="J_showdialog" title="'.L('edit').'">'.L('edit').'</a>|
                                <a href="javascript:;" data-acttype="ajax" data-uri="'.U('depart/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'" class="J_confirmurl" style="color:#F76741" title="'.L('delete').'">'.L('delete').'</a>';
    ...
    public function _before_add(){
        ...
        $this->assign('depart_type',C('depart_type'));
    }
    ...
    {
        $id = I('id', '', 'intval');
        $depart = $this->_mod->field('id,pid')->where(array('id' => $id))->find();
        $depart_spid = M('depart')->where(array('id' => $depart['pid']))->getField('spid');
        if ($depart_spid == 0) {
            $depart_spid = $depart['pid'];
        } else {
            $depart_spid .= $depart['pid'];
        }

        $this->assign('spid', $depart_spid);
        $this->assign('depart_type',C('depart_type'));
    }
    ...
    protected function _before_update($data) {
        ...
        $depart = $this->_mod->field('pid')->where(array('id'=>$data['id']))->find();
        if ($data['pid'] != $depart['pid']) {
    ...
    public function get_city_info(){
        ...
        //$url = 'http://api.map.baidu.com/depart/v2/suggestion?query='.$kw.'&region=136&output=json&ak='.C('baidu_lbs_key');
