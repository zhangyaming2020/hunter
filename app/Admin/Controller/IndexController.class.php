...
class IndexController extends AdminCoreController {
	...
	public function index() {
        ...
        $pid=$top_menus[0]['id'];//页面载入时左侧默认的菜单
        foreach($top_menus as $k=>$v){
        	$top_menus[$k]['child_menu']=$this->_mod->admin_menu($v['id']);
        }
        //dump($top_menus);
		...
		$my_admin = array('username'=>$_SESSION['admin']['username'], 'rolename'=>$rolename,'name'=>$name);
		//dump($my_admin);exit;
        
        //是否有订单提醒权限
        $is_remind = D('Admin_auth')->where(array('menu_id'=>497,'role_id'=>$_SESSION['admin']['role_id']))->count();

        $this->assign('my_admin', $my_admin);
        $this->assign('is_remind', $is_remind);
        $this->display();
        //$Home = A('Home/Home');
        //$Home->auto();
    }
...
public function test() {
    ...
    public function panel() {
		...
		$rolename=M('admin_role')->where(array('id'=>$_SESSION['admin']['role_id']))->getField('name');
		$my_admin = array('username'=>$_SESSION['admin']['username'],'nickname'=>$_SESSION['admin']['nickname'], 'rolename'=>$rolename,'name'=>$name);
		//dump($my_admin);
		//统计
		...
		$map['_string']=$field;
		$data['project']=M('project')->where($map)->count();//全部项目
		unset($map['_string']);
		$map['owner_id'] = $share_ids;
		$data['custom']=M('custom')->where($map)->count();//全部客户
		$data['resume']=M('resume')->where($map)->count();//全部简历
		$map['recommend_id']=$_SESSION['admin']['id'];//
		unset($map['owner_id']);
		$data['report']=M('report')->where($map)->count();//全部报告
		unset($map['recommend_id']);
		$map['creator_id']=$_SESSION['admin']['id'];//
		$data['offer']=M('offer')->where($map)->count();//全部报告
		
		//面试的report
		$whe['status']=2;
		if($_SESSION['admin']['id']!=1){//项目下的推荐报告
				$whe['recommend_id']=$_SESSION['admin']['id'];//
        }
        
        //待处理的offer
        if($_SESSION['admin']['id']!=1){
        $ma['creator_id'] = $_SESSION['admin']['id'];
        }
        //dump($ma);
        $ma['oftaState']=array('in',array(0,1));
        $offer=D('Offer')->where($ma)->relation(true)->select();
        //dump(D('Offer')->getLastSql());
        $interview=D('Report')->where($whe)->relation(true)->select();
		$info=M('Admin')->where(array('id'=>$_SESSION['admin']['id']))->find();
		$status=array('等待通知候选人','候选人、客户协商中','成功-待审核','成功-待付款','已完成','失败','失败','审核不通过');
        $this->assign('status',$status);
		$this->assign('admin',$my_admin);
    ...
    //每日门店订单量统计
	public function store_order($wh,$store_info,$total){
		$str='';
		foreach($store_info as $k =>$v){
			$wh['store_id']=$v['id'];
			$str .=''.$v['title'].': '.M('order')->where($wh)->count().'<br/>';
		}
		$str .='总量: '.$total;
		return $str;
	}
	 //每日门店订单金额统计
	public function order_money($wh,$store_info){
		$str='';
		foreach($store_info as $k =>$v){
			$wh['store_id']=$v['id'];
			//$wh['status']=array('',array());
			$str .=''.$v['title'].': '.M('order')->where($wh)->sum('totalprices').'<br/>';
		}
		//dump($str);
		return $str;
	}
	 //每日门店销售量
	public function store_sell($wh,$store_info){
		$str='';
		foreach($store_info as $k =>$v){
			$wh['o.store_id']=$v['id'];
			//$wh['status']=array('',array());
			$str .=''.$v['title'].': '.M('order_list')->alias('ol')->where($wh)
			->join("__ORDER__ o on o.id=ol.oid")->sum('ol.nums').'<br/>';
		}
		//dump($str);
		return $str;
	}
...
//          }
            ...
            M('Admin')->where(array('id'=>$admin['id']))->setInc('login_nums',1);
