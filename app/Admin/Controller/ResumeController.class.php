...
class ResumeController extends AdminCoreController {
    ...
    public function _before_index() {
		...
		$admin=M('admin')->where(array('status'=>1))->field('id,nickname')->select();
    	//dump($admin);
    	$this->_before_add();
    	$str=M('resume')->where(array('id'=>52))->getField('info');
		...
		$year=1900;
		for($i=0;$i<95;$i++){
			$array1[$i]=$year+$i+'.';//点号匹配一个字符
		}
	  	$year=implode('|',$array1);
	  	//$str="就职于湖北省安装公司从事水电安装工作。";
  	$pattern = '/("'.$p_str.'")(.*)(?)有限公司/'; 
	...
	protected function _search() {
		...
		($owner_id = I('owner_id','',  'trim')) && $map['owner_id']=$owner_id;
		if($owner_id){
			$uname=M('admin')->where(array('id'=>$owner_id))->getField('nickname');
		}
    ...
    public function _before_update($data){
    	if(in_array($data['file_type'],array('xls','xlsx'))){
    		unset($data['info']);
    	}
    	
    	if(in_array($data['file_type'],array('doc','docx'))&&$data['pdf_name']){
    		$html=code2html($data['info']);
    		$string=strip_tags(substr($html,strpos($html,'</head>')+7));
	        $string=preg_replace('/&nbsp;/is', '', $string);
	        $data['info']=DeleteHtml($string);
    	}
    	return $data;
    }
    public function _after_update($id){
    	...
    	if(in_array($type,array('xls','xlsx'))){
    		$info&&file_put_contents('./uploads/resume/'.$name.'.html',code2html($info));
    	}
    	
    	if(in_array($type,array('doc','docx'))){
    		$name=I('pdf_name');
    		$info=I('info');
    		$name&&$info&&file_put_contents('./uploads/htm/'.$name,code2html($info));
    	}
    }
    ...
    public function detail_info(){
    	...
    	if(in_array($info['file_type'],array('doc','docx'))){
    		$info['download_name']=$info['org_name'];
    	}
        $this->assign('resume_id',$id);
        $this->assign('admin',$admin);
	...
	public function doc_import_project() {
    	if(IS_POST){
    		exit;//见 file_upload
    	}
    	else{
    ...
    public function ajax_check_name() {
   	...
   	public function file_upload(){
		...
		if ($filename) {
		    ...
		    if($str=='pdf'){
		    	$name=md5(time()).'.pdf';
		    	$up_name="./uploads/resume/pdf/" . $name;
		    ...
		    if($res){
	    		...
	    		if($str=='pdf'){//pdf转txt
	    			...
	    			$da['info']=$html=file_get_contents('./uploads/resume/excel/'.$name.'.html');
	    			//$html=file_get_contents("./uploads/resume/excel/9ad978883d0fec92e8262c0eb1894eed.html");
			        //$content = preg_replace("/height=.+?['|\"]/i",'',$html);//去除样式  
			        $string=strip_tags(substr($html,strpos($html,'</head>')+7));
			        $string=preg_replace('/&nbsp;/is', '', $string);
			        $res['html']=DeleteHtml($string);
	    		}
	    		else{
	    			include "./doc.php";
			...
			//dump($res);exit;
	    		...
	    		$map=array();
				$res['email']&&$data['resuEmail']=$map['resuEmail']=$res['email'];
				$res['mobile']&&$data['resuContactInfo']=$map['resuContactInfo']=$res['mobile'];
				if($map){//查重
					$find=M('resume')->where($map)->getField('id');
					if($find){
						@unlink($up_name);//删除文件
						IS_AJAX && $this->ajax_return(2, '简历已存在,请不要重复导入',U('Resume/detail',array('id'=>$find)));
					}
				}
	...
	public function file_upload_project(){
   		$filename =$resuname= iconv('UTF-8', 'GB2312', $_FILES['file']['name']); 
   		//dump($_FILES["file"]['name']);exit;
		$key = $_POST['key'];
		$key2 = $_POST['key2'];
		if ($filename) {
		    ...
		    if($str=='pdf'){
		    	$name=md5(time()).'.pdf';
		    	$up_name="./uploads/resume/pdf/" . $name;
		    	$res_up=move_uploaded_file($_FILES["file"]["tmp_name"],"./uploads/resume/pdf/" . $name);//如何保存成功，则进行下一步
		    	//dump(array($res_up,$name));exit;
		    }
		    elseif(strtolower($str)=='xls' || $str=='xlsx'){//excel转html
		    	$name=md5(time());
		    	$up_name="./uploads/resume/excel/".$name.'.'.$str;
		    	$res_up=move_uploaded_file($_FILES["file"]["tmp_name"],"./uploads/resume/excel/".$name.'.'.$str);//如何保存成功，则进行下一步
		    	
	    	}
		    else{
		    	$up_name="./uploads/resume/" . $filename;
		    	$res_up=move_uploaded_file($_FILES["file"]["tmp_name"],"./uploads/resume/" . $filename);//如何保存成功，则进行下一步
			//dump($res_up);exit;
			}
		    if($res_up){
				// 取文件后缀名
	    		...
	    		if($str=='pdf'){//pdf转txt
		    		...
		    		$name=md5(time());
	    			$res=$this->exceltohtml($name,$str);
	    			$html_path='./uploads/resume/excel/'.$name.'.html';
	    			$da['info']=$html=file_get_contents('./uploads/resume/excel/'.$name.'.html');
	    			//$html=file_get_contents("./uploads/resume/excel/9ad978883d0fec92e8262c0eb1894eed.html");
			        //$content = preg_replace("/height=.+?['|\"]/i",'',$html);//去除样式  
			        $string=strip_tags(substr($html,strpos($html,'</head>')+7));
	    		...
	    		else if(in_array($str,array('htm','mht','html','shtml'))){//html
		    		include "./doc.php";
	        		$res=word_upload("/uploads/resume/".$filename);	    		}
	    		else{
	    			$name=iconv("GB2312","UTF-8",$filename);
	    			$new_name=md5(time());
	    			$up_result=$this->wordtohtml($name,$new_name);
	    			if(!$up_result){
	    				@unlink($up_name);
	    				IS_AJAX && $this->ajax_return(0, L('operation_failure'));
	                	$this->error(L('operation_failure'));
	    			}
	    			$html_path='./uploads/htm/'.$new_name.'.html';
	    			$da['info']=$html=file_get_contents('./uploads/htm/'.$new_name.'.html');
	    			$string=strip_tags(substr($html,strpos($html,'</head>')+7));
			        $string=preg_replace('/&nbsp;/is', '', $string);
			        $res['html']=$str_html=DeleteHtml($string);
			        
			        preg_match('/(男|女)/',$str_html, $sex);
			 		preg_match('/(初中|中专|大专|专科|高中|本科|硕士|博士)/',$str_html, $education);
			 		$email=$this->getEmail($str_html); 
			 		$mobile=$this->findThePhoneNumbers($str_html);
			 		$res['mobile']=$mobile[0];
			 		$res['sex']=$sex[0];
			 		$res['email']=$email[0];
			 		$edu=array('初中'=>1,'高中'=>2,'专科'=>3,'大专'=>4,'本科'=>5,'硕士'=>6,'博士'=>7);
			 		$res['educations']=$edu[$education[0]];
			 		$res['pdf_name']=$new_name.'.html';
			 		$res['org_name']=$name;
			...
			//dump($res);exit;
				...
				$map=array();
				if(!$res['email']&&!$res['mobile']){//没有联系方式不能入库
					@unlink($up_name);//删除文件
					if($html_path){
						@unlink($html_path);//删除文件
					}
					if($str=='doc' || $str='docx'){//删除临时文件
						@unlink("./uploads/temp/".$new_name.".".$str);//删除文件
						if($str=='doc'){
							@unlink("./uploads/temp/".$new_name.".docx");//删除文件
						}
					}
					IS_AJAX && $this->ajax_return(0, L('operation_failure'));
				}
...
//					}
	...
	public function wordtohtml($filename,$new_name){
		$file_name=$filename;
		$str=strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
		$name=str_replace('.'.$str, '', $file_name);
		$md5_name=$new_name;
		$path=$file="/home/mutong/uploads/htm/".$md5_name.".html";
		$name = iconv('UTF-8', 'GBK', $name);
		fopen($path,"w");
		$word="/home/mutong/uploads/resume/".$name.".".$str;
		$new_name="/home/mutong/uploads/temp/".$md5_name.".".$str;
		$res=1;
		if($str=='docx'){
			$res=copy($word,"/home/mutong/uploads/temp/".$md5_name.".".$str);
			$save_name=$new_name;
			exec("python /home/mutong/convert.py $path $save_name");
		
		}
		else if($str=='doc'){
			$doc_name=$md5_name.".".$str;
			$res=copy($word,"./uploads/temp/".$doc_name);
			exec("C:\Python27\python D:\phpstudy\PHPTutorial\WWW\mtrc\\doc.py $md5_name");
		}
		
		/*
		if($str=='docx'){
			$res=copy($word,"./uploads/temp/".$md5_name.".".$str);
			$save_name=$new_name;
		}
		else if($str=='doc'){
			$doc_name=$md5_name.".".$str;
			$res=copy($word,"./uploads/temp/".$doc_name);
			$save_name=str_replace('.doc', '.docx', $new_name);
			$docx_name=str_replace('.doc', '.docx', $doc_name);
			exec("C:\Python27\python D:\phpstudy\PHPTutorial\WWW\mtrc\\docx.py $doc_name  $docx_name",$out);
		}
		$res&&exec("C:\Python27\python D:\phpstudy\PHPTutorial\WWW\mtrc\\convert.py $path $save_name",$out);
		return $out;*/
		if(file_exists('./uploads/htm/'.$md5_name.'.html')){
        	$res=file_get_contents('./uploads/htm/'.$md5_name.'.html');
			if($res){
//				preg_match_all("/<img(.*)src=\"([^\"]+)\"[^>]+>/isU", $res,$matches);
//				if($matches[2]){
//					exec("C:\Python27\python D:\phpstudy\PHPTutorial\WWW\mtrc\\token.txt $md5_name");//保存图片
//				}
	...
	public function pdftotext($file_name,$name){
		...
		//dump(array($file_name,$name));
		$content = shell_exec("C:\\xpdf\pdftotext -layout $dir_name  && echo  1 ");
		//dump(array($name,$content));exit;
		$data=array();
		if($content==1){//数据整理
			$str="";
