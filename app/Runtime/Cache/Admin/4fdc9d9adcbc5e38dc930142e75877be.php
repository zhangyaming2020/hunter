...
<body style="background:#fff;padding:10px;">
	<form id="info_form" name="info_form" action="<?php echo u('linkman/edit');?>" method="post">
		<input type="hidden" name="cucoId" value="0">
		<input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
		<input type="hidden" name="cucoCustId" value="161">
		<div class="form_default"  style="padding:0; height:435px; OVERFLOW-Y: auto;">
			<p>
				<label for="cucoName">姓名</label>
				<input type="text" name="cucoName" id="cucoName" class="sf" value="<?php echo ($info["cuconame"]); ?>">
			</p>
			<p>
				<label for="cucoSex" class="nopadding">性别</label>
				<input type="radio" name="cucoSex" value="1" <?php if($info['cucosex'] == 1): ?>checked<?php endif; ?>> 女 &nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="cucoSex" value="2" <?php if($info['cucosex'] == 2): ?>checked<?php endif; ?>> 男
			</p>
			<p>
				<label for="cucoAge">年龄</label>
				<input type="text" name="cucoAge" id="cucoAge" class="sf" style="width:120px;" value="<?php echo ($info["cucoage"]); ?>">
			</p>
			<p>
				<label for="cucoFunction">职能/称谓</label>
				<input type="text" name="cucoFunction" id="cucoFunction" class="sf" style="width:150px;" value="<?php echo ($info["cucofunction"]); ?>"> 如：人事部经理
			</p>
			<?php if(is_array($dic)): $i = 0; $__LIST__ = $dic;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><p>
				...
				<?php if(is_array($val['sub'])): $i = 0; $__LIST__ = $val['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if($info['cutaimporta'] == $v['id'] or $info['cutaapprove'] == $v['id']): ?>selected="selected"<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
			</p><?php endforeach; endif; else: echo "" ;endif; ?>
			<p>
				<label for="cucoOfficeTel">办公电话</label>
				<input type="text" name="cucoOfficeTel" id="cucoOfficeTel" class="sf" value="<?php echo ($info["cucoofficetel"]); ?>">
			</p>
			<p>
				<label for="cucoFax">传真</label>
				<input type="text" name="cucoFax" id="cucoFax" class="sf" value="<?php echo ($info["cucofax"]); ?>">
			</p>
			<p>
				<label for="cucoMobile">手机</label>
				<input type="text" name="cucoMobile" id="cucoMobile" class="sf" value="<?php echo ($info["cucomobile"]); ?>">
			</p>
			<p>
				<label for="cucoEmail">邮箱</label>
				<input type="text" name="cucoEmail" id="cucoEmail" class="sf" value="<?php echo ($info["cucoemail"]); ?>">
			</p>
			<p>
				<label for="cucoQq">QQ</label>
				<input type="text" name="cucoQq" id="cucoQq" class="sf" value="<?php echo ($info["cucoqq"]); ?>">
			</p>
			<p>
				<label for="cucoMsn">MSN</label>
				<input type="text" name="cucoMsn" id="cucoMsn" class="sf" value="<?php echo ($info["msn"]); ?>">
			</p>
			<p>
				<label for="cucoHomeTel">家庭电话</label>
				<input type="text" name="cucoHomeTel" id="cucoHomeTel" class="sf" value="<?php echo ($info["cucohometel"]); ?>">
			</p>
			<p>
				<label for="cucoAboutKey">关心重点</label>
				<textarea name="cucoAboutKey" class="mf" rows="" cols="" style="height:60px; width:250px;"><?php echo ($info["cucoaboutkey"]); ?></textarea>
			</p>
			<p>
				<label for="cucoInterest">利益诉求</label>
				<textarea name="cucoInterest" class="mf" rows="" cols="" style="height:60px; width:250px;"><?php echo ($info["cucointerest"]); ?></textarea>
			</p>
			<p>
				<label for="cucoRemarker">备注</label>
				<textarea name="cucoRemarker" class="mf" rows="" cols="" style="height:60px; width:250px;"><?php echo ($info["cucoremarker"]); ?></textarea>
			</p>
		</div>
	</form>
