...
<input type="hidden" name="fuctions" id="fuctionHidden" value="2232a2233a2234">
	...
	<div class="notification msginfo" style="margin-bottom: 10px">您可以多选或一次性Ctrl+A全选某个文件夹下的简历文件进行批量导入，一次最多可导入10000份简历，支持导入的简历格式：word、excel、pdf、htm、mht、html、shtml、txt、wps文档。
...
$(function(){
	var up = $('#upload').Huploadify({
		buttonText : '请选择简历',
		auto:true,
		fileTypeExts:'*.doc;*.docx;*.htm;*.mht;*.html;*.pdf;*.shtml;*.txt;*.xls;*.xlsx;*.wps',
		multi:true,
		...
		uploader:"<?php echo U('Resume/file_upload');?>",
				...
				var str='';
					str+='<div id="file_upload_1-queue" class="uploadify-queue">';
						str+='<div id="SWFUpload_0_4" class="uploadify-queue-item uploadify-error">';					
							...
							str+='<a href="javascript:;">X</a></div><span class="fileName">'+subString(file.name,20)+'……'+' ('+$('#'+'fileupload_1_'+file.index).find('.totalsize').text()+')</span><span class="data"> - 邮箱[ 未知 ]已存在，您已经录入过此简历&nbsp;&nbsp;&nbsp;<a href="javascript:;" style="color:#f00;"     class="J_showdialog" data-uri="'+info.data+'" data-title="编辑简历" data-id="edit" data-editor="1" data-width="783" data-height="500">显示</a>&nbsp;&nbsp;&nbsp;<span style="color:#00f;">未更新</span></span><div class="uploadify-progress"><div class="uploadify-progress-bar" style="width: 1px;"><!--Progress Bar--></div></div></div></div>';
							str+='</div>';
						str+='</div>';
					str+='</div>';
				$('#'+'fileupload_1_'+file.index).find('.up_filename').html(subString(file.name,37,1)+$('#'+'fileupload_1_'+file.index).find('.totalsize').text()+'- 邮箱[ 未知 ]已存在，您已经录入过此简历'+'&nbsp;&nbsp;&nbsp;<a href="javascript:;" style="color:#f00;"     class="J_showdialog" data-uri="'+info.data+'" data-title="编辑简历" data-id="edit" data-editor="1" data-width="783" data-height="500">显示</a>&nbsp;&nbsp;&nbsp;<span style="color:#00f;">未更新</span>');
