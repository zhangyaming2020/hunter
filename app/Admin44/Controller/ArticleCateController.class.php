...
use Admin\Org\Tree;
class ArticleCateController extends AdminCoreController {
    public function _initialize() {
        parent::_initialize();
        $this->_mod = D('ArticleCate');
        $this->set_mod('ArticleCate');
    }
    ...
    public function ajax_getchilds() {
        $id = I('id','', 'intval');
        $return = $this->_mod->field('id,name')->where(array('pid'=>$id,'id'=>array('neq',40)))->select();
        if ($return) {
