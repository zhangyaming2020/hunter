...
class TicketsController extends AdminCoreController {
    ...
    public function _initialize() {
		...
		$this->_rel_mod = D('TicketsType');


        $this->set_mod('Tickets');
    ...
    public function tickets_cate() {
        ...
        $result = $this->_cate_mod->where(array('id|pid'=>145))->order($sort . ' ' . $order)->select();
    ...
    public function _before_index() {
        ...
        $res = M('tickets_cate')->field('cate_id,title')->select();
        ...
        foreach ($res as $val) {
            ...
            $cate_list[$val['cate_id']] = $val['title'];



            $big_cate2[$val['id']] = M('tickets_cate') -> where(array('id'=>$val['cate_id'])) ->getField('title');
...
//	print_r($res);
	...
	public function _before_add(){
		$res =$this->_rel_mod->where(array('status'=>1))->field('id,name')->select();
		$this->assign('res',$res);
	}
...
//			}
            ...
            $cate=M('tickets_cate')->field('id,title')->select();
    ...
    public function edit() {
        ...
        if (IS_POST) {
			...
			if($_POST['tp_rel']){
				$data['tp_rel']=implode(',',$_POST['tp_rel']);
			}
            $data['tj']=I("post.tj");
            ...
            $img_list = M('item_img')->where(array('item_id'=>$id,'type'=>2))->select();
...
//			   $data[$k][img] = $goods_info['img']; //商品图片
    ...
    public function indexs(){
        ...
        $lists=M('tickets_cate')->order('id desc')->select();
    ...
    public function adds(){
        ...
        if(IS_POST){
            ...
            if (false === $data = D('tickets_cate')->create()) {

                IS_AJAX && $this->ajax_return(0, $mod->getError());

                $this->error(D('tickets_cate')->getError());
    ...
    public function edits(){
        ...
        if(IS_POST){
            ...
            if (false === $data = D('tickets_cate')->create()) {
                ...
                $img=M('tickets_cate')->where(array('id'=>$id))->getField('img');
...
//                  );
                ...
                M('item_img')->where(array('item_id'=>$id))->delete();

                  $item_imgs && M('item_img')->addAll($item_imgs);

                    D('tickets_cate')->where(array('id'=>$id))->save($data);
            ...
            $info=M('tickets_cate')->where(array('id'=>$id))->find();

            $place_spid = D('Place')->where(array('id'=>$info['place_id']))->getField('spid');

            if( $place_spid==0 ){

                $place_spid = $info['place_id'];
			...
			$res =M('spot_type')->where(array('status'=>1))->field('id,name')->select();
			$this->assign('spot_type',$res);
    ...
    public function shan(){
        ...
        M('tickets_cate')->where(array('id'=>$id))->delete();
