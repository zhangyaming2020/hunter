...
class PlayController extends AdminCoreController {
...
//	print_r($res);
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            //上传图片
			$label1 =implode('|',I('label1'));
			$data['label1']=$label1;
			$label2 =implode(',',I('label2'));
			$data['label2']=$label2;
...
//			}
			...
			$store=array();
			foreach($res as $k=>$v){
				$store[$v['id']]=M('store')->where(array('shop_type'=>$v['id']))->field('id,title')->select();
			}
			
			$this->assign('store',json_encode($store));
            $this->display();
    ...
    public function edit() {
        ...
        if (IS_POST) {
           ...
           //玩乐清单
		   	$attr = I('attr');
            if($attr){
				$ids =M('play_menu')->where(array('item_id'=>$item_id))->getField('id',true);
				$new_ids=array();
                ...
                foreach( $attr['name'] as $key=>$val ){					
                    if($val&&$attr['price'][$key]&&$attr['num'][$key]){
						...
						if($attr['attr_id'][$key]){
							...
							$res =M('play_menu')->where(array('id'=>$attr['attr_id'][$key]))->save($atr);
						}
						else{
							$res =M('play_menu')->add($atr);
						}
                    }
                }
			...
			//dump(code2html($item['menu']));
            //清单
            $attr_sx=M('play_menu')->where("item_id=".$id." and pid=0")->select();
			//dump($attr_sx);
			...
			$stores =M('store')->where(array('shop_type'=>$item['cate_id']))->field('id,title')->select();
			//dump($stores);
			$this->assign('now_stores',$stores);
