...
class ArticleController extends AdminCoreController {
    ...
    protected function _search() {
        ...
        $selected_ids = '';
        $map['cate_id']=array('not in','40');
        if (!in_array($cate_id,array('40')) && $cate_id) {
    ...
    protected function _before_update($data) {
		if($data['cate_id']==18||$data['cate_id']==30){
			empty($data['uid'])&&$this->error('请填写用户ID,以便识别');
			$id =M('member')->where(array('id'=>$data['uid']))->getField('id');
			(!$id)&&$this->error('用户不存在');
		}
        if (!empty($_FILES['img']['name'])) {
