...
use Admin\Org\Tree;
class TicketsTypeController extends AdminCoreController {

    public function _initialize() {

        parent::_initialize();

        $this->_mod = D('TicketsType');

        $this->_cate_mod = D('TicketsType');

        $this->set_mod('TicketsType');
    ...
    public function index() {
        ...
        foreach($result as $r) {
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('tickets_type/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="500" data-height="20">'.L('edit').'</a> |
                                <a href="javascript:;" class="J_confirmurl" data-acttype="ajax" data-uri="'.U('tickets_type/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>';
