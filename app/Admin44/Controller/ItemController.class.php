...
//	print_r($res);
    ...
    public function add() {
        ...
        if (IS_POST) {
			...
			$item_type=I('item_type','','intval');
			$item_type==1&&$data['is_group']=1;
			$item_type==2&&$data['is_jf']=1;
			$item_type==3&&$data['is_norm']=1;
			//添加商品
...
//              }
            ...
            $item_imgs&& M('main_item_img')->addAll($item_imgs);
			//商品编码
...
//			}
            $attr = I('attr');
            if($attr){
				//M('item_attr')->where(array('item_id'=>$item_id))->delete();
                foreach( $attr['name'] as $key=>$val ){					
                    if($val&&$attr['price'][$key]&&$attr['num'][$key]){
						$atr['item_id'] = $item_id;
						$atr['attr_name'] = $val;
						$atr['attr_num'] = $attr['num'][$key];
						$atr['attr_price'] = $attr['price'][$key];
						$atr['act_price'] = $attr['act_price'][$key];
						$atr['act_nums'] = $attr['act_nums'][$key];
						$atr['attr_integral'] = $attr['integral'][$key];
						$atr['jf_kucun'] = $attr['jf_kucun'][$key];
						$atr['sales'] = $attr['sales'][$key];
						$atr['act_sales'] = $attr['act_sales'][$key];
						$atr['jf_sales'] = $attr['jf_sales'][$key];
						$atr['quota'] = $attr['quota'][$key];
						M('main_item_attr')->add($atr);
                    }
                }
            }
			//exit;
			...
			$ids = $this->_cate_mod->where(array('id'=>array('not in','190,164'),'pid'=>143))->field('id,name')->select();
           
            $this->assign('ids',$ids); //分类选中
            $this->display();
        }
    }
...
//              }
			...
			$old_imgs=M('main_item_img')->where(array('item_id'=>$item_id))->getField('url',true);
            //更新图片和相册
			if($imgs&&$old_imgs){
				M('main_item_img')->where(array('item_id'=>$item_id))->delete();
				/*foreach($old_imgs as $k=>$v){
					$str1 =mb_substr($v,0,strlen($v)-4);
					$str2=mb_substr($v,-4,4);
					@unlink(attach($v,'item'));
					@unlink('data/attachment/item/'.$str1.'_b'.$str2);
					@unlink('data/attachment/item/'.$str1.'_m'.$str2);
					@unlink('data/attachment/item/'.$str1.'_s'.$str2);
				}*/
			}
			//dump($item_imgs);exit;
            $item_imgs && M('main_item_img')->addAll($item_imgs);
 ...
 //附加属性
            $attr = I('attr');
            if($attr){
				$ids =M('main_item_attr')->where(array('item_id'=>$item_id))->getField('id',true);
				$new_ids=array();
				//M('item_attr')->where(array('item_id'=>$item_id))->delete();
                foreach( $attr['name'] as $key=>$val ){					
                    if($val&&$attr['price'][$key]&&$attr['num'][$key]){
						...
						$atr['quota'] = $attr['quota'][$key];
						if($attr['attr_id'][$key]){
							$new_ids[]=$attr['attr_id'][$key];
							M('main_item_attr')->where(array('id'=>$attr['attr_id'][$key]))->save($atr);
						}
						else{
							M('main_item_attr')->add($atr);
						}
				...
				$diff_ids = array_diff($ids, $new_ids);
				$diff_ids&&M('main_item_attr')->where(array('id'=>array('in',$diff_ids)))->delete();
            }
            $this->success(L('operation_success'));
        } else {
            $id = I('id',0,'intval');
			...
			$new_spid =explode('|',$spid);array_shift($new_spid);
			$this->assign('spid',implode('|',$new_spid)); //分类选中
            $tag_cache = unserialize($item['tag_cache']);
            $item['tags'] = implode(' ', $tag_cache);
			$item['yhq'] = explode(',', $item['isyhq']);
            $this->assign('info', $item);
            //相册
            $img_list = M('main_item_img')->where(array('item_id'=>$id))->select();
            $this->assign('img_list', $img_list);			
			//类别属性
			$attr_sx=M('main_item_attr')->where("item_id=".$id." and pid=0")->select();
			//dump($attr_sx);
			$this->assign('attr_sx', $attr_sx);
            //属性
            $attr_list = M('main_item_attr')->where(array('item_id'=>$id))->select();
    ...
    public function delete()
    {
        ...
        $ids = trim(I($pk), ',');
        if ($ids) {
            if (false !== $mod->delete($ids)&&false !== D('main_item_img')->delete($ids)) {
