...
use Admin\Org\Tree;
class TrialCateController extends AdminCoreController {
    public function _initialize() {
        parent::_initialize();
        $this->_mod = D('TrialCate');
        $this->set_mod('TrialCate');
    }
    ...
    public function index() {
        ...
        $array = array();
        foreach($result as $r) {
            $r['str_img'] = $r['img'] ? '<span class="img_border"><img src="'.attach($r['img'], 'trial_cate').'" style="width:26px; height:26px;" class="J_preview" data-bimg="'.attach($r['img'], 'trial_cate').'" /></span>' : '';
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('trial_cate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_item_cate').'" data-id="add" data-width="520" data-height="360">'.L('add_item_subcate').'</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="'.U('trial_cate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="520" data-height="360">'.L('edit').'</a> |
                                <a href="javascript:;" class="J_confirmurl" data-acttype="ajax" data-uri="'.U('trial_cate/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>';
    ...
    public function ajax_upload_img() {
        ...
        if (!empty($_FILES['img']['name'])) {
            $result = $this->_upload($_FILES['img'], 'trial_cate', array(
                    'width'=>21,
                    'height'=>21,
                    'remove_origin' => true,
                )
