...
class ItemCateController extends AdminCoreController {
    ...
    public function index() {
        ...
        $array = array();
        foreach($result as $r) {
			$r['str_bg_img'] = $r['bg_img'] ? '<span class="img_border"><img src="'.attach($r['bg_img'], 'item_cate').'" style="width:26px; height:26px;" class="check-image" data-bimg="'.attach($r['bg_img'], 'item_cate').'" /></span>' : '';
            ...
            $r['str_index'] = '<img data-tdtype="toggle" data-id="'.$r['id'].'" data-field="is_index" data-value="'.$r['is_index'].'" src="'.C('TMPL_PARSE_STRING.__STATIC__').'/images/toggle_' . ($r['is_index'] == 0 ? 'disabled' : 'enabled') . '.gif" />';
