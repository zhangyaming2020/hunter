...
class HotelController extends AdminCoreController {
...
//			}
            ...
            $data['attr_oldprice']=I('attr_oldprice');

            $data['attr_integral']=I('attr_integral');

            $data['attr_num']=I('attr_num');

            $data['attr_remark']=I('attr_remark');

            $data['attr_ydxz']=I('attr_ydxz');
...
//              }
          ...
          M('hotel')->where(array('id'=>$item_id))->save($data);
         ...
         //   M('item_img')->where(array('item_id'=>$item_id))->delete();
            ...
            if (!empty($_FILES['imgsss']['name'])) {

                $result = $this->_upload($_FILES['imgsss'], 'item/'.$date_dir, array(
                ...
                if ($result['error']) {
                    ...
                    $datas['img'] = $date_dir . $result['info'][0]['savename'];
				}
            }
            $datas['attr_name']=I('attr_name');

            $datas['attr_value']=I('attr_value');

            $datas['attr_price']=I('attr_price');

            $datas['attr_oldprice']=I('attr_oldprice');

            $datas['attr_integral']=I('attr_integral');

            $datas['attr_num']=I('attr_num');

            $datas['attr_remark']=I('attr_remark');

            $datas['attr_ydxz']=I('attr_ydxz');

            $datas['wf']=I('wf');

            $datas['item_id']=I('item_id');
			...
			//dump($datas);exit;
            if($datas['attr_name']){
                $aa=M('item_attr')->add($datas);

            }

                $this->success(L('operation_success'),'/jradmin.php?m=admin&c=hotel&a=index&menuid=370');
                ...
                $place_spid = D('Place')->where(array('id'=>$item['place_id']))->getField('spid');

                if( $place_spid==0 ){

                    $place_spid = $item['place_id'];
                ...
                $img_list = M('item_img')->where(array('item_id'=>$id,'type'=>3,'hotel_type'=>0))->select();

                $this->assign('img_list', $img_list);

                $img_list1 = M('item_img')->where(array('item_id'=>$id,'type'=>3,'hotel_type'=>1))->select();

                $this->assign('img_list1', $img_list1);

                $img_list2 = M('item_img')->where(array('item_id'=>$id,'type'=>3,'hotel_type'=>2))->select();

                $this->assign('img_list2', $img_list2);

                $img_list3= M('item_img')->where(array('item_id'=>$id,'type'=>3,'hotel_type'=>3))->select();

                $this->assign('img_list3', $img_list3);

                $img_list4 = M('item_img')->where(array('item_id'=>$id,'type'=>3,'hotel_type'=>4))->select();

                $this->assign('img_list4', $img_list4);
                ...
                $ca=M('hotel')->where(array('id'=>$id))->getField('cate_id');

                $attr= M('attr')->where(array('status'=>1,'inv_status'=>$ca))->select();
...
//			   $data[$k][img] = $goods_info['img']; //商品图片
    ...
    public function shanchu(){
        ...
        $me=M('item_attr')->where(array('id'=>$id))->delete();
    ...
    public function bianji(){
        if(IS_POST){
            ...
            if (!empty($_FILES['img']['name'])) {
				...
				//$data['img'] = M('item')->where(array('id'=>$data['id']))->getField('img');
				}
            $data['attr_name']=I('attr_name');
            ...
            $data['zc']=I('zc');
            M('item_attr')->where(array('id'=>$id))->save($data);
            $itemid = M('item_attr')->where(array('id'=>$id))->getField('item_id');
                $this->success(L('operation_success'),'/jradmin.php?m=admin&c=hotel&a=edit&id='.$itemid.'&menuid=370');

        }else{

            $id=I('id');

            $mem=M('item_attr')->where(array('id'=>$id))->find();
			//dump($mem);exit;
            $this->assign('mem',$mem);
    ...
    public function cates(){
        ...
        if(IS_POST){
            ...
            $mem=M('attr')->where(array('inv_status'=>$id))->field('id,name')->select();
    ...
    public function hotel_order(){
        ...
        $list = D('hotel_order')
            ...
            ->field('order_sn,id,member_id,name,mobile,create_time,item_id,man,amount,data2,data1')
        ...
        foreach($list as $k=>$v){
            ...
            $list[$k]['title']=M('hotel')->where(array('id'=>$v['item_id']))->getField('title');

            $list[$k]['data1']=substr($list[$k]['data1'], 0, -3);

            $list[$k]['data2']=substr($list[$k]['data2'], 0, -3);
    ...
    public function xtxs(){
        ...
        M('hotel_order')->where(array('id'=>$id))->delete();
