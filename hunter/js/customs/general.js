...
$(document).ready(function(){
	...
	**/
			...
			$.get(url+"?rand="+new Date(),function(data){
				...
				if(url=="follow/remind"){
					$('.dropbox').find("li").bind("click",function(){
						...
						var detailDialog = $.dialog.open("http://ee.headhunterclub.com/follow/show/" + id,{
							title:	title,
							...
							button:[{
								...
								callback: function () {
						        	var _confirm = $.dialog.confirm('你确认要删除此日历吗？', function(){
						        		$.post("http://ee.headhunterclub.com/follow/delete/"+ id +"?rand="+new Date(), function(result){
						        			if(result=='success'){
						        				clearMessageNum();
						        				_confirm.close();
				        						detailDialog.close();
								...
								name: '已处理',
								callback: function () {
						        	$.post("http://ee.headhunterclub.com/follow/state/"+ id +"?rand="+new Date(), function(result){
					        			if(result=='success'){
					        				clearMessageNum();
			        						detailDialog.close();
					        			}else{
	...
	$(document).click(function(event) {
		if(!$(event.target).is('.dropbox') && $(event.target).parents(".dropbox").length==0) {
			hideTopPopup();
		}
	});
	...
	if(msg!="updatePatch"){
		if(msg.indexOf("error")!=-1){
			...
			window.setInterval(function(){
				$.get("http://ee.headhunterclub.com/serverRestartState?rand" + new Date(),function(result){
					if(result.indexOf("登录")>-1){
						top.location="http://ee.headhunterclub.com/login?flag=ot&r="+new Date();
					}
				});
			},1000);
		}else{
...
function portalResumeShow(id){
	var detailDialog = $.dialog.open("http://ee.headhunterclub.com/project/portal/resumeShow/" + id,{
		title:	"外投简历详细",
...
function projectReportDetailPop(id){
	var detailDialog = $.dialog.open("http://ee.headhunterclub.com/project/report/show/" + id,{
		title:	"推荐报告",
...
function showPluginService(alias, id){
	var mydialog = $.dialog.open("http://ee.headhunterclub.com/showPluginServices/"+ id +"?rand=" + new Date(),{
...
function showLogs(){
	$.dialog.open("http://ee.headhunterclub.com/userlogs/main?rand=" + new Date(),{
		id:'user_login_logs',
		fixed: true,
		title :	'登录日志',
		width : '720px',
